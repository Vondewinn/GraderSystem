...
public class CanDataUtils {

    public static int[] handleCanData(byte[] bytes) {
        byte[] id = new byte[4];
        System.arraycopy(bytes, 1, id, 0, id.length);
        byte[] data = null;
        ...
        switch (frameFormatType) {
            case 0:
                frameFormat = 0;
                frameType = 0;
                extendid = (((((id[0] & 0xff) << 24) | ((id[1] & 0xff) << 16) | ((id[2] & 0xff) << 8) | ((id[3] & 0xff))) & 0xFFFFFFFFl) >> 21);
            ...
            case 4:
                frameFormat = 1;
                frameType = 0;
                extendid = (((((id[0] & 0xff) << 24) | ((id[1] & 0xff) << 16) | ((id[2] & 0xff) << 8) | ((id[3] & 0xff))) & 0xFFFFFFFFl) >> 3);
        ...
        int[] dataArray = new int[16];
        dataArray[0] = (int) extendid;
        for (int i = 0; i < data.length; i++) {
            dataArray[i + 1] = data[i] & 0xFF;
        }

        return dataArray;
        //return ("通道【" + bytes[0] + "】," + (frameFormat == 0 ? "标准帧" : "扩展帧") + " " + (frameType == 0 ? "数据帧" : "远程帧") + " id:[0x" + String.format("%08x", extendid) + "] " + (data == null ? "" : "data:[0x" + saveHex2String(data) + "]"));
    }
